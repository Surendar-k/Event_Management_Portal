...
exports.getFullEvent = async (req, res) => {
  try {
    ...
    const [[event]] = await db.execute("SELECT * FROM events WHERE event_id = ?", [event_id]);
    if (!event) return res.status(404).json({ error: "Event not found" });

    const [[eventInfo]] = await db.execute("SELECT * FROM event_info WHERE event_id = ?", [event_id]);
    const [agenda] = await db.execute("SELECT * FROM agenda WHERE event_id = ?", [event_id]);
    const [[financialPlanning]] = await db.execute("SELECT * FROM financial_planning WHERE event_id = ?", [event_id]);
    const [[foodTravel]] = await db.execute("SELECT * FROM food_travel WHERE event_id = ?", [event_id]);
    const [checklist] = await db.execute("SELECT * FROM checklist WHERE event_id = ?", [event_id]);
